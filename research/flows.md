...
flowchart TD
    ...
    PL == продовжити ==> ED["📱 Редактор"]:::screen
    PL -. новий проект .-> IMP["📱 Вибір відео"]:::screen

    IMP == вибрала ==> Q3{Файл OK?}:::decide
    IMP -. скасувала .-> PL
    Q3 -. помилка .-> IMP_E["🔴 Помилка імпорту"]:::state
...
**Стани**
- ⬜ empty — Список проектів: порожній стан з CTA «Новий проект» (blob-перевірка IndexedDB — прозора для юзера)
- ⏳ loading + 🔴 error — Вибір відео: завантаження → валідація (непідтримуваний формат / >500 MB / пошкоджено) → «Спробувати інший файл»
- ✓ autosave — Редактор: silent toast «Збережено» (2 сек) або «Не вдалось зберегти» + retry
- 👁 preview — Екран Експорту: preview кадрування вбудований — оновлюється live при виборі формату
- 🔴 error — Рендер: помилка + «Повторити» або «Повернутись до редактора»
- 📤 share — Поділитись / завершити: system share sheet · список проектів · новий проект

**Тупики**
- ✓ виправлено: Рендер зависає без виходу → `REND -. скасувати .-> EXP`
- ✓ виправлено: Рендер падає і юзер відмовляється повторювати → закрити без повтору повертає до ED

**Примітки**
- Autosave тригер: після кожної зміни на таймлайні
- Типи помилок імпорту та blob-відновлення задокументовані в sitemap.md
- Додати кліп з Редактора → повторний вхід до IMP (вторинна дія, не показана на схемі)
- SHARE: post-success навігація — share sheet · список проектів · новий проект
...
flowchart TD
    ...
    ED["📱 Редактор · є відео, мему ще немає\n(editor-no-meme · старт R3)"]:::screen --> QC{Є кліп?}:::decide
...
**Стани**
- старт R3 = `editor-no-meme.html` — редактор з відео, мему ще немає, [Додати мем] активний
...
- лише 9:16 / 1:1 у MVP — 16:9 взагалі не пропонується (інша ніша, серверний рендер), тож окремої підказки «не підтримується» немає
- 👁 preview — вбудований в Екран Експорту: оновлюється live при виборі формату
